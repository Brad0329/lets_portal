...
function renderNavbar() {
    ...
    const menuItems = [
        ...
        { href: '/bid-list.html', label: '입찰 예정', icon: '📌', show: true },
        { href: '/excluded-list.html', label: '제외 공고', icon: '🚫', show: true },
        { href: '/settings.html', label: '설정', icon: '⚙️', show: isAdmin || hasPermission('display') || hasPermission('keyword') || hasPermission('org') },
        { href: '/source-list.html', label: '공고수집 출처', icon: '📡', show: isAdmin || hasPermission('keyword'), className: 'nav-link-bottom' },
    ];

    const topItems = menuItems.filter(m => m.show && !m.className);
    const bottomItems = menuItems.filter(m => m.show && m.className === 'nav-link-bottom');

    const renderLink = (m) => {
        const active = page === m.href || (m.href === '/index.html' && page === '/');
        return `<a href="${m.href}" class="nav-link ${active ? 'active' : ''}">${m.icon} <span class="nav-label">${m.label}</span></a>`;
    };

    const topHtml = topItems.map(renderLink).join('');
    const bottomHtml = bottomItems.map(renderLink).join('');
    ...
    nav.innerHTML = `
        <div class="sidebar-inner">
            ...
            <div class="sidebar-menu">${topHtml}</div>
