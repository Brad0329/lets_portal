...
## 세션 6 — 보고서 뷰 추가 (2026-04-20)

편집 뷰 중심의 claim 라이브러리를 **승인된 claim만 깔끔하게 출력하는 읽기용 보고서 뷰**로 전환하는 기능 추가. 툴바 토글 방식(옵션 1) + 인쇄용 @media print CSS(옵션 3) 병행.

### 결정사항 (세션 6)

1. **옵션 3 + 옵션 1 병행 채택, 옵션 2(별도 페이지) 기각** — 별도 페이지(`/claim-report.html`)는 API·라우트·네비 추가 비용이 드는데, 보고서는 설정 페이지 맥락(claim 라이브러리 편집 중)에서 바로 열어보는 게 자연스러움. 툴바 토글로 mode switch + `@media print`로 인쇄 최적화 = 두 마리 토끼.

2. **승인된 claim만 자동 필터** — 보고서 뷰 진입 시 `_currentVerified`를 강제로 1로 바꾸지 않고 별도 엔드포인트 호출(`/claims?verified=1`)로 전체 로드 후 클라이언트에서 `user_verified===1` 필터. 이유: 편집 뷰 필터 상태를 오염시키지 않고 복귀 시 그대로 복원 가능.

3. **카테고리 정렬은 CATEGORIES 상수 순서(Identity→Methodology→USP→Philosophy→Story→Operational)** — 제안서 흐름과 유사한 순서로 고정. 카테고리 내부는 recurrence DESC → confidence DESC (여러 제안서에 반복된 주장이 회사 정체성의 중심이라는 설계 §3-3 Core Identity 관점).

4. **숨김은 `body.extractor-report-mode` 클래스 + CSS로** — JS가 개별 element에 `style.display='none'` 주입하면 복귀 시 일일이 복원해야 함. body 클래스 토글 한 줄로 스크린 모드 ↔ 보고서 모드 전환.

5. **`@media print`와 body 클래스 셀렉터를 분리** — 둘이 섞이면 "화면 보고서 뷰"와 "실제 인쇄"를 독립 조정할 수 없음. 예: 보고서 뷰는 클레임 카드 테두리를 살짝 남기지만 인쇄는 더 얇게 하는 식의 미세조정이 가능.

### 실패한 접근 (세션 6)

1. **`<header class="report-header">` 태그 사용** → 실패
   - 원인: settings.html 상단에 `<style>header { display: none; }</style>` 룰이 있어서 내가 생성한 보고서 헤더가 **전부 사라짐**. getBoundingClientRect에서 height=0으로 진단되기 전까지 "스크롤 문제"로 오판했음.
   - 해결: `<header>` → `<div class="report-header">`, `<footer>` → `<div class="report-footer">`. 의미론보다 기존 CSS 충돌 회피가 우선.

### 구현 항목 (세션 6)

- [x] `frontend/settings.html` — `#company-profile-section` id 부여, `extractor-sources-wrap` 래퍼 추가, 보고서 뷰 + @media print CSS 블록 삽입
- [x] `frontend/js/extractor.js` — `_reportMode`·`_reportPrevFilters`·`_reportClaims`·`_sourcesCache` 상태, `renderReportView`·`renderReportClaim`·`loadReportClaims`·`extractorToggleReportView`·`extractorPrintReport` 함수 추가. `renderToolbar`에 모드 분기
- [x] E2E: 토글 → 31 claim × 6 카테고리 렌더 → 편집 뷰 복귀 시 필터 복원(미검토 5) → `@media print` 11개 규칙 정상 로드 확인

## 세션 7 — C-5-B 회사 프로필 자동 채우기 (2026-04-20)

> 설계 문서: `work_log2/design_phase_c5b_profile_autofill.md`

claim/힌트를 쌓기만 하고 정량 프로필로 내려보낼 통로가 없었던 **라스트마일**을 채움. 회사 프로필 섹션 상단에 "제안서로 프로필 자동 채우기" 카드를 두고, 업로드→분석→**미리보기 모달에서 선택 반영** 흐름으로 정량 프로필과 역량 자산을 동시에 채우는 통합 UX.

### 결정사항 (세션 7)

1. **기존 역량 자산 탭 유지, 상단에 신규 카드 추가** — 탭을 대체하지 않음. "첫 세팅/빠른 업데이트"는 상단 카드, "개별 claim 세밀 편집"은 역량 자산 탭이라는 역할 분리. 탭의 승인/병합/편집 기능을 버리는 비용이 너무 큼.

2. **자동 반영 금지 — 미리보기 모달 필수** — 제안서엔 오탐·과장이 섞임. 로그에도 "사업자 등록 여부"가 등록 힌트로 오탐된 케이스 기록. UI에서 체크박스 기본값은 "중복·오탐 아닌 것" + "claim 신뢰도 0.9+"로 설정해 사용자 부담 최소화하면서도 마지막 확정은 사용자에게.

3. **merge_mode는 2개만 — `fill_empty`와 `append_only`** — `overwrite`는 기존 JSON 배열을 통째 교체해 수동 입력을 날리는 위험 동작이라 MVP 제외. 실무 대부분 케이스는 두 모드로 충분.

4. **`patents_certs`는 JSON 배열이 아니라 textarea** — 실제 스키마 확인 결과 `project_history`·`staff_list`만 JSON 배열이고 `patents_certs`는 단일 textarea. MVP는 줄 단위 append, fuzzy similarity(0.85)로 중복 감지. JSON 배열화는 별건.

5. **힌트 type 매핑 — 실적 vs 인증/자격만 반영, 나머지 20종은 ignored** — 실제 데이터에서 type은 19종까지 분화(목표·예산·상금·성과지표 등). `company_profile` 스키마 필드에 매핑되는 건 실적→`project_history`·인증/등록/자격/특허→`patents_certs`뿐. 나머지는 모달에 "매핑 없음" 접힘 박스로 표시해 투명성 유지, DB 변경 없음.

6. **`openPreview`를 window에 노출 — 테스트 + 재보기 기능 겸용** — 업로드 안 하고 기존 소스 미리보기를 다시 여는 정당한 사용자 기능이기도 함. 테스트 목적으로만 노출하는 dev flag보다 깔끔.

7. **apply 후 `loadProfile()` + `initExtractor()` 동시 호출** — 정량 프로필 탭과 역량 자산 탭 둘 다 리프레시. 두 UI가 독립적으로 자기 데이터를 다시 로드하므로 apply 엔드포인트는 DB 쓰기만 책임, 렌더링은 각 탭 모듈에 위임.

### 실패한 접근 (세션 7)

1. **uvicorn StatReload가 Windows에서 파일 수정을 즉시 반영 안 함** → 404 오해
   - 원인: `StatReload detected changes. Reloading...` 메시지 후 실제 프로세스 재시작까지 수 초~수십 초 지연. 그 사이 요청하면 **기존 코드**가 응답하므로 새 엔드포인트는 404.
   - 해결: 리로드 감지 로그 확인 후에도 응답이 맞지 않으면 `preview_stop` + `preview_start`로 재시작. 이번엔 코드 오류 아니라 타이밍이었지만 디버깅 시 재시작이 가장 확실.

### 구현 항목 (세션 7)

- [x] `backend/routers/extractor.py` — 헬퍼(`_norm_text`, `_is_suspicious`, `_is_duplicate_project`, `_is_duplicate_cert`, `_hint_to_project_item`) + 엔드포인트 2개(`GET /preview/{src_id}`, `POST /apply-to-profile`)
- [x] `frontend/settings.html` — 상단 카드 + 미리보기 모달 HTML, `autofill.js` 스크립트 로드
- [x] `frontend/js/autofill.js` 신규 — 업로드, 폴링, preview 렌더, 체크박스/병합옵션, apply, 리프레시까지 전체 흐름
- [x] 매핑 규칙: 실적(14) → project_history, 인증·등록·인증/등록·자격·특허(10) → patents_certs, 나머지(20) → ignored
- [x] 브라우저 검증: 카드·모달 렌더, 기본 체크박스 상태(실적 14/14·인증 9/10·claim 30/36), X·ESC·배경 클릭 닫기, apply 실제 반영 후 원복

### 검증 결과 (src_0002 기준)

| 항목 | 값 |
|------|-----|
| 미리보기 API | project 14 / cert 10 / ignored 20 / claims 36 |
| 기본 체크 상태 | project 14 / cert 9 (오탐 1 해제) / claim 30 (신뢰도 0.9+) |
| apply 테스트 (힌트 2건) | project_history +2, hints_marked_imported=2, 성공 |
| 원복 | project_history=`[]`, patents_certs=`""`, imported=0 정상 복귀 |
