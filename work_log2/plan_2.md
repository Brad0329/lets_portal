...
##### 단계별 로드맵
- [ ] **D-1 현황 진단** — 샘플 10건을 현 파서에 돌려 실패 패턴 분류. 파싱 결과 시각화 CLI 작성.
- [ ] **D-2 목차 탐지 프로토타입** — 1~5페이지 스캔 + 목차 항목 추출 + 본문 매칭 로직.
- [ ] **D-3 Haiku 섹션 분할** — BaseAIClient 인터페이스에 Haiku 추가. 마커 패턴 추출 프롬프트 설계. 프롬프트 캐싱 적용.
- [ ] **D-4 Flat chunk 폴백** — 문단 경계 유지한 20K자 청크 분할.
- [ ] **D-5 검증 자동화** — Regression 테스트(10건 모두 섹션 ≥ 5개 + claim ≥ 10건). Ground truth는 1차 자동 추출 → 사용자 확인으로 구축.
- [ ] **D-6 UI 반영** — 분석 완료 후 품질 배지 표시, claim=0 경우 경고 배너.

##### 성공 기준
- 샘플 10건 모두 섹션 최소 5개 이상 추출
