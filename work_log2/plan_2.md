...
#### C-8: 범용 제안서 파서 (Proposal Parser v2) — 긴급 재설계 2026-04-20

> 상세 설계: `work_log2/design_phase_d_parser.md`

##### 배경 — Phase C-5 기술 부채 발견

C-5-B 실전 투입 중 **현 파서가 경과원 ESG 제안서 1종 형식에만 동작함**이 확인됨. 신규 제안서(혁신창업스쿨) 업로드 시 `_segment_text()`가 섹션 0개를 반환 → LLM 호출 0회 → claim 0건 추출. 에러 없이 "정상 완료"로 끝나 더 은밀한 실패.

**근본 원인:**
- 섹션 분할 정규식이 `Ⅰ./Ⅱ./Ⅲ.` + `^[1-9]\s+[가-힣]` 2개만 지원
- 설계 문서에 명시된 "폴백2: 글자수 기반 청크"가 **실구현 누락**
- MVP 검증을 **단일 샘플**로 마감해서 일반화 실패 사전 감지 못함

##### 목표
다양한 형식의 제안서(HWP/HWPX/PDF/DOCX/PPTX)에서 섹션 구조를 안정적으로 추출. 경과원 형식 의존 제거.

##### 접근 전략 — 하이브리드 3단계 폴백 (결정)
1. **1차: Python 목차 탐지** — 1~5페이지에서 "목차 / 목 차 / CONTENTS / 차례" 찾기 → 목차 항목을 ground truth로 본문 매칭. 비용 0, 높은 정확도.
2. **2차: Haiku 섹션 분할** — 목차 실패 시 문서 첫 2~3페이지를 Haiku에 보내 마커 패턴 판별 → Python이 정규식으로 분할. 제안서당 100~300원.
3. **3차: Flat chunk 폴백** — 그래도 실패 시 20K자 청크로 강제 분할 + UI에 경고 배너.

**claim 추출은 Sonnet 유지.** Haiku 품질 실험은 별건(Phase C-5 후반 과제로 이미 등록).

##### 샘플 데이터 (확보 완료)
`data/proposal_sample/` 10건 — HWP 9 + PPTX 1, 1.4MB~21MB, 모두 (주)렛츠 제안서.
발주기관: 경기기술창업·혁신창업스쿨·창업역량강화·한국관광공사·충남콘텐츠·한국저작권위원회·원광대·경남귀어인·산림분야 청년창업·경과원 ESG.

##### 단계별 로드맵
- [x] **D-1 현황 진단** — 샘플 10건을 현 파서에 돌려 실패 패턴 분류. `tools/check_parser.py` 작성. 조용한 실패 2건 + PPTX 1건 + 마커 변형 미지원 확인. (2026-04-22)
- [x] **D-2 목차 탐지 프로토타입** — `backend/extractors/segmenter/` 패키지 구축. 토큰 기반 엔트리 파서 + 3단계 본문 매칭. **매칭률 90.4%** (설계 목표 60% 크게 초과). 조용한 실패 2건 전부 해결. GT Editor에 "D-2 목차 불러오기" 버튼 추가. (2026-04-22)
- [x] **PPTX 처리 정책** — 미지원 확정. `file_parser.py` 에서 명시적 에러 반환. PDF/DOCX 변환 안내. (2026-04-22)
- [ ] **D-3 Haiku 섹션 분할** — BaseAIClient 인터페이스에 Haiku 추가. 마커 패턴 추출 프롬프트 설계. 프롬프트 캐싱 적용. *(D-2 성능이 목표 초과로 필요성 재평가 필요)*
- [ ] **D-4 Flat chunk 폴백** — 문단 경계 유지한 20K자 청크 분할.
- [ ] **D-5 검증 자동화** — Regression 테스트(10건 모두 섹션 ≥ 5개 + claim ≥ 10건). Ground truth는 1차 자동 추출 → 사용자 확인으로 구축 (`tools/gt_editor/`).
- [ ] **D-6 UI 반영** — 분석 완료 후 품질 배지 표시, claim=0 경우 경고 배너.

##### 성공 기준
- 샘플 10건 모두 섹션 최소 5개 이상 추출 — **9 HWP 13+섹션 달성, PPTX 1건 미지원 처리**
- 샘플 10건 모두 claim 10건 이상 추출 — *(D-5 에서 측정)*
- 제안서당 평균 비용 600원 이하 — *(D-5 에서 측정)*
- 실패 시 UI에서 명확한 원인 안내 — *(D-6)*
...
- `work_log2/log_phaseD.md` — Phase D-1 (현황 진단) + D-2 (목차 탐지 프로토타입, 매칭률 90.4%)
...
- `work_log2/design_phase_d_parser.md` — Phase C-8 (범용 제안서 파서 v2, 긴급 재설계)
