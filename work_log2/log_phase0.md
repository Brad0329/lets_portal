...
### 로고 적용
- 원본 로고 파일(`img/lets_logo.png`) → `frontend/img/lets_logo.png`로 복사하여 사용
- 로그인 페이지: CSS filter로 파란색 처리 (밝은 배경)
- 네비게이션 바: 원본 흰색 로고 그대로 사용 (어두운 배경)

### 조회기간/정렬 개선
- "조회 기간 (일)" → "입찰공고 최근 (일)" 라벨 변경
- 입찰공고일(`start_date`) 기준 최근 N일 필터링 구현
- "최신순" → "입찰공고일 최신순" 라벨 변경, `start_date DESC` 정렬

### 추가 변경 파일
| 파일 | 변경 유형 |
|------|----------|
| `frontend/img/lets_logo.png` | 신규 — 로고 이미지 |
| `frontend/login.html` | 수정 — 로고 이미지 적용 |
| `frontend/js/auth.js` | 수정 — 네비게이션 바 로고 이미지 |
| `frontend/css/navbar.css` | 수정 — 로고 이미지 스타일 |
