...
#### 5-1. 배포 준비
- 환경변수 설정 (API 키 등)
- SQLite → 배포 환경에 맞게 데이터 마이그레이션
- requirements.txt 최종 정리
- CORS 설정 확인

#### 5-2. 배포 실행
- 배포 플랫폼 선택 (Render / Railway / Oracle Cloud 등)
- 서버 설정 + 도메인 연결
- 스케줄러 정상 작동 확인
- 최종 테스트
...
### Phase 1 (완료)
- [x] **Step 1**: 프로젝트 구조 생성 + .env 설정 (API 키)
- [x] **Step 2**: DB 스키마 생성 + 엑셀 데이터 임포트 + 설정 초기값 삽입
- [x] **Step 3**: K-Startup API 수집기 개발 + 테스트
- [x] **Step 4**: 중소벤처기업부 API 수집기 개발 + 테스트
- [x] **Step 5**: FastAPI 백엔드 (검색/필터 + 설정 API)
- [x] **Step 6**: 프론트엔드 메인 페이지 + 기관 바로가기

### Phase 2 (완료)
- [x] **Step 7**: 설정 페이지 (키워드 ON/OFF + 조회조건)
- [x] **Step 8**: 상세보기 모달 + 반응형 UI
- [x] **Step 9**: 키워드 추가/삭제 기능
- [x] **Step 10**: 자동 수집 스케줄러 (APScheduler)

### Phase 3 (완료)
- [x] **Step 11**: DB 스키마 확장 (9개 컬럼 추가)
- [x] **Step 12**: 수집기 매핑 수정 (새 필드 수집)
- [x] **Step 13**: 프론트엔드 상세 모달 확장
- [x] **Step 14**: 나라장터 API 테스트 → 서버 미복구 (500), 코드 준비 완료
- [x] **Step 15**: 재수집 실행 (K-Startup 267건 확장 필드 반영)

### Phase 4
- [ ] **Step 16**: 첨부파일 다운로드 기능 (중기부 우선)
- [ ] **Step 17**: 추가 기능 (마감임박 하이라이트 등)

### Phase 5
- [ ] **Step 18**: 클라우드 배포 + 최종 테스트
